"""Rename RPE assessment boolean columns to the CRA Art. 3(2) inclusion criteria names.

Replaces the old browser-centric decision tree fields with the four CRA Art. 3(2)
inclusion criteria used for RDPS classification. Also adds provider_is_nis2_msp to
support guidance text after classification.

Old → New:
  has_downloadable_component    → is_developed_by_manufacturer      (criterion 1)
  is_browser_only_access        → has_bidirectional_exchange        (criterion 4)
  is_essential_to_core_function → is_necessary_for_product_function (criterion 2)
  is_manufacturer_owned         → directly_interacts_with_product   (criterion 3)
...
def upgrade() -> None:
    # Rename the four decision-tree boolean columns to match the CRA Art. 3(2) criteria 1-4.
    op.execute(
